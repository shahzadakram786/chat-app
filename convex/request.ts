...
export const create = mutation({
    ...
    handler:async (ctx, args)=>{
        ...
        const requestAlredyReceived = await ctx.db.query("requests").withIndex("by_receiver_sender" , (q) => q.eq("receiver",currentUser._id).eq("sender",receiver._id)).unique();
        ...
        if(friends1.some((friend)=> friend.user2 === receiver._id)
         || friends2.some((friend)=> friend.user1 === receiver._id)){
            ...
            throw new ConvexError("You are alredy freinds with this user")
        }
